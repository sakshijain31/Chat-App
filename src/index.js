...
// configure server to work with socketio
...
// socket is an object and it contains information abt connections
io.on('connection', (socket) => {
    console.log('New Websocket Connection')

    // server (emit) -> client (receive) - countUpdates
    ...
    socket.on('sendMsg', (msg, callback) => {
        ...
        //event emit to every single connection
    ...
    socket.on('disconnect', () => {
        ...
        if (user) {
            io.to(user.room).emit('message', generateMessages('Admin', `The ${user.username} has left`))
            io.to(user.room).emit('roomData', {
                room: user.room,
                users: getUsersInRoom(user.room)
            })
